        ...
        public static async Task SeedAsync(IdentityServerContext accountContext)
        {
            ...
            if (!accountContext.Users.Any())
            {
                accountContext.Users.Add(new User()
                    { Id = Guid.NewGuid(), Email = "Admin@gmail.com", Name = "Mr Admin",Password = PasswordHash.Hash("Admin1234")});
            ...
            {
                var admin =await accountContext.Users.Where(x => x.Email == "Admin@gmail.com" && x.Name == "Mr Admin").SingleAsync();
                var roles = await accountContext.Roles.ToListAsync();
                foreach (var role in roles)
                {
                    await accountContext.UserRoles.AddAsync(new UserRole() { Id = Guid.NewGuid(), RoleId = role.Id, UserId = admin.Id });
                }
